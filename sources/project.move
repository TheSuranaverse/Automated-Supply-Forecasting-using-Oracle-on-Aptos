module SupplyOracle::SupplyForecasting {
    ...
    public entry fun initialize(
        ...
        // Create the forecast event handle
        let forecast_events = account::new_event_handle<ForecastUpdateEvent>(account);
        
        // Emit initial forecast event
        event::emit_event(&mut forecast_events, ForecastUpdateEvent {
            product_id,
        ...
        let store = SupplyForecastStore {
            oracle_data,
            forecasted_supply: initial_supply, // Initialize with current supply as forecast
    ...
    public entry fun update_forecast(
        ...
        assert!(current_supply > 0, error::invalid_argument(E_ZERO_SUPPLY));
    ...
    /// Get the current forecasted supply for a product (view function)
    public fun get_forecasted_supply(owner_address: address): u64 acquires SupplyForecastStore {
        assert!(exists<SupplyForecastStore>(owner_address), error::not_found(E_NOT_INITIALIZED));
        let store = borrow_global<SupplyForecastStore>(owner_address);
        store.forecasted_supply
    }
}